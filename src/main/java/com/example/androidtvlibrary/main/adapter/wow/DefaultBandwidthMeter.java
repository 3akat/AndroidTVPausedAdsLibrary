...
//        eventDispatcher.removeListener(eventListener);
    ...
    @Override
    public void onTransferInitializing(DataSource source, DataSpecTest dataSpec, boolean isNetwork) {
        // Do nothing.
    }

    @Override
    public synchronized void onTransferStart(DataSource source, DataSpecTest dataSpec, boolean isNetwork) {
        if (!isNetwork) {
            return;
        }
        if (streamCount == 0) {
    ...
    @Override
    public synchronized void onBytesTransferred(DataSource source, DataSpecTest dataSpec, boolean isNetwork, int bytes) {
        if (!isNetwork) {
            return;
        }
    ...
    @Override
    public synchronized void onTransferEnd(DataSource source, DataSpecTest dataSpec, boolean isNetwork) {
        if (!isNetwork) {
            return;
        }
