...
/**
 * Implements the internal behavior of {@link WowPlayerImpl}.
 */
public final class WowPlayerImplInternal
    ...
    public void setPlaybackParameters(com.example.androidtvlibrary.main.adapter.wow.PlaybackParameters playbackParameters) {
    ...
    @Override
    public void onPlaybackParametersChanged(com.example.androidtvlibrary.main.adapter.wow.PlaybackParameters playbackParameters) {
        ...
        try {
            switch (msg.what) {
                ...
                case MSG_SET_PLAYBACK_PARAMETERS:
                    setPlaybackParametersInternal((com.example.androidtvlibrary.main.adapter.wow.PlaybackParameters) msg.obj);
                    break;
            ...
            stopInternal(
                    ...
                    /* resetPositionAndState= */ false,
                    /* acknowledgeStop= */ false);
            playbackInfo = playbackInfo.copyWithPlaybackError(new Exception());
            maybeNotifyPlaybackInfoChanged();
        }
        return true;
    }
    ...
    private String getExoPlaybackExceptionMessage(Exception e) {
//        if (e.type != Exception.TYPE_RENDERER) {
...
//                + RendererCapabilities.getFormatSupportString(e.rendererFormatSupport);
    ...
    private void setPlayWhenReadyInternal(boolean playWhenReady) throws Exception {
        rebuffering = false;
        this.playWhenReady = playWhenReady;
        if (!playWhenReady) {
            stopRenderers();
    ...
    private void seekToCurrentPosition(boolean sendDiscontinuity) throws Exception {
        ...
        if (newPositionUs != playbackInfo.positionUs) {
            ...
            if (sendDiscontinuity) {
                playbackInfoUpdate.setPositionDiscontinuity(TestPlayer.DISCONTINUITY_REASON_INTERNAL);
            }
        }
    }

    private void startRenderers() throws Exception {
        rebuffering = false;
        mediaClock.start();
    ...
    private void stopRenderers() throws Exception {
        mediaClock.stop();
        for (Renderer renderer : enabledRenderers) {
    ...
    private void updatePlaybackPositions() throws Exception {
    ...
    private void doSomeWork() throws Exception, IOException {
...
//        TraceUtil.endSection();
    ...
    private void seekToInternal(SeekPosition seekPosition) throws Exception {
        ...
        try {
            ...
            if (seekPositionAdjusted) {
                playbackInfoUpdate.setPositionDiscontinuity(TestPlayer.DISCONTINUITY_REASON_SEEK_ADJUSTMENT);
            }
        }
    }
    ...
    private void resetRendererPosition(long periodPositionUs) throws Exception {
    ...
    private void setPlaybackParametersInternal(com.example.androidtvlibrary.main.adapter.wow.PlaybackParameters playbackParameters) {
    ...
    private void sendMessageInternal(PlayerMessage message) throws Exception {
    ...
    private void sendMessageToTarget(PlayerMessage message) throws Exception {
    ...
    private void deliverMessage(PlayerMessage message) throws Exception {
    ...
    private void ensureStopped(Renderer renderer) throws Exception {
        if (renderer.getState() == Renderer.STATE_STARTED) {
            renderer.stop();
        }
    }

    private void disableRenderer(Renderer renderer) throws Exception {
        mediaClock.onRendererDisabled(renderer);
        ensureStopped(renderer);
        renderer.disable();
    }

    private void reselectTracksInternal() throws Exception {
    ...
    private void handleSourceInfoRefreshed(MediaSourceRefreshInfo sourceRefreshInfo)
        ...
        MediaSource.MediaPeriodId newPeriodId = playbackInfo.periodId;
    ...
    private void updatePeriods() throws Exception, IOException {
        if (mediaSource == null) {
    ...
    private void maybeUpdateLoadingPeriod() throws Exception, IOException {
    ...
    private void maybeUpdateReadingPeriod() throws Exception {
        MediaPeriodHolder readingPeriodHolder = queue.getReadingPeriod();
    ...
    private void maybeUpdatePlayingPeriod() throws Exception {
        ...
        while (shouldAdvancePlayingPeriod()) {
            ...
            int discontinuityReason =
                    oldPlayingPeriodHolder.info.isLastInTimelinePeriod
                            ? TestPlayer.DISCONTINUITY_REASON_PERIOD_TRANSITION
                            : TestPlayer.DISCONTINUITY_REASON_AD_INSERTION;
    ...
    private void handlePeriodPrepared(MediaPeriod mediaPeriod) throws Exception {
    ...
    private void sendPlaybackParametersChangedInternal(
            com.example.androidtvlibrary.main.adapter.wow.PlaybackParameters playbackParameters, boolean acknowledgeCommand) {
        handler
                .obtainMessage(
    ...
    private static final class PlaybackInfoUpdate {
        ...
        private @TestPlayer.DiscontinuityReason int discontinuityReason;
        ...
        public void setPositionDiscontinuity(@TestPlayer.DiscontinuityReason int discontinuityReason) {
            if (positionDiscontinuity
                ...
                Assertions.checkArgument(discontinuityReason == TestPlayer.DISCONTINUITY_REASON_INTERNAL);
                return;
            }
